...
// END stuff that is in is-any-array
...
// Stuff that is in ml-array-rescale
...
// END tuff that is in ml-array-rescale
...
function findRootTrustRegion(
	...
	let convergence = !checkTol(r, tol);
...
function check_quantile_input(x1, p1, x2, p2, n_checks, var_name, quantile_setter_div) {
...
function quantile_setter_callback(
    quantile_setter_div,
    x1_box,
    p1_box,
    x2_box,
    ...
    let input_ok = check_quantile_input(theta1, p1, theta2, p2, 1, 'θ', quantile_setter_div);
    ...
    if (input_ok) {
        ...
        function quantilePickBeta(x1, p1, x2, p2) {
            ...
            console.log(quantileRootFindFunBeta);
            [logParams, convergence] = findRootTrustRegion(quantileRootFindFunBeta, guess, args=args);

            return [Math.exp(logParams.get(0, 0)), Math.exp(logParams.get(1, 0))], convergence;
        }

        let [[alpha, beta], convergence] = quantilePickBeta(theta1, p1, theta2, p2);

        let text;
        if (!convergence) {
            text = '<p>α = ' + alpha.toPrecision(4) + '</p>';
            text += '<p>β = ' + beta.toPrecision(4) + '</p>';
        } else {
...
if (quantile_setter_button.active === 0) {
    quantile_setter_callback(quantile_setter_div, x1_box, p1_box, x2_box, p2_box, sliders);
}
